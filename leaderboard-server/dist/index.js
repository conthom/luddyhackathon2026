...
import { countActiveEntries, deleteEntriesByUser, deleteEntryById, getDbPathForLog, getLeaderboardTop, getRaceState, insertEntry, loadActiveEntries, loadHistoryFiltered, replaceEntireActiveLeaderboard, replaceLittle500Entries, setRaceState, } from "./db.js";
...
import { histogram, mean, median, percentileRank, quantile, sortedNumbers, standardDeviation, } from "./stats.js";
...
app.get("/leaderboard", (req, res) => {
    ...
    if (asHtml) {
        const rows = top
            .map((r) => `<tr><td>${r.rank}</td><td>${escapeHtml(displayTeamUser(r.user))}</td><td>${r.score}</td><td>${escapeHtml(r.submittedAt)}</td></tr>`)
            .join("");
...
server.on("error", (err) => {
    if (err.code === "EADDRINUSE") {
        console.error(`Port ${PORT} is already in use. Stop the other server on this port or run with a different PORT (e.g. PORT=3002).`);
    }
    else {
        console.error(err);
    }
